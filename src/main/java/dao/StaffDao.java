...
public class StaffDao implements CRUD<Staff> {
    ...
    public List<Staff> getAll() {
        ...
        try (Connection connection = Connect_MySQL.getConnect()) {
            ...
            while (resultSet.next()) {
                ...
                staffList.add(new Staff(id, name, address,  phoneNumber,email, salary, department));
            }
    ...
    public boolean edit(int id, Staff staff) {
        String sql = "UPDATE nhanvien SET name = ?, " +
                "address = ?,phone = ?,mail = ?, mail = ?,salary = ?, phongban=? WHERE (id = ?)";
        try (Connection connection = Connect_MySQL.getConnect()) {
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            ...
            preparedStatement.setString(4, staff.getMail());
    ...
    public Staff findById(int id) {
        ...
        try (Connection connection = Connect_MySQL.getConnect()) {
            ...
            return new Staff(idStaff, name, address, phoneNumber, email, salary, department);
